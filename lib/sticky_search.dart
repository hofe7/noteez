...
Future<SearchResult> searchStickies(
  ...
  if (q.isEmpty) {
    final recent = [...stickies]
      ..sort((a, b) => b.contentUpdatedAt.compareTo(a.contentUpdatedAt));
  ...
  if (range != null) {
    final hits =
        stickies
            .where(
              (s) =>
                  range.contains(s.createdAt) ||
                  range.contains(s.contentUpdatedAt),
            )
            .toList()
  ...
  final related = <Sticky>[for (final e in cands.take(relatedMax)) e.key];
