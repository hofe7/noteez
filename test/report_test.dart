...
void main() {
  test('moving a window does not count old work as recently completed', () {
    final now = DateTime(2026, 9, 5);
    final note = makeSticky(
      x: 0,
      y: 0,
      blocks: [todoBlock('old task', true)],
    ).copyWith(updatedAt: now, contentUpdatedAt: DateTime(2025));
    final report = buildReport([note], now);
    expect(report.week.activeStickies, 0);
    expect(report.week.completed, isEmpty);
  });
  ...
  test('activeStickies = 기간 내 수정된 스티커 수', () {
    final recent = _sticky([
      const TextBlock(id: 'r', text: 'a'),
    ], updatedMs: DateTime(2026, 6, 9).millisecondsSinceEpoch);
    final old = _sticky([
      const TextBlock(id: 'o', text: 'b'),
