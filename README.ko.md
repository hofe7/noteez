...
![여러 종류의 메모를 묶어 보여주는 Noteez 전체 보기](docs/images/overview-en.png)

> 영어 예제와 번역 문구를 적용해 실제 UI를 렌더링한 소개용 화면입니다. 현재 배포 앱 UI는 한국어입니다.
...
이미지는 앱 전용 로컬 저장소에 보관됩니다. 검색과 추천도 로컬에서 실행됩니다.

로컬 저장은 앱 수준 암호화나 같은 사용자 권한의 다른 프로그램으로부터의 격리를 뜻하지는 않습니다. [보안 안내](SECURITY.md)를 참고해 주세요.
...
첫 공개 베타에는 로컬에서 검증한 build 9 DMG를 게시했습니다. 이후 새 버전의 자동 배포 절차는 다음과 같습니다.

1. `pubspec.yaml`의 앱 버전을 올리고 변경을 `main`에 반영합니다.
2. 앱 버전과 같은 태그를 만들어 push합니다. 빌드 번호(`+10`)는 태그에서 제외합니다.

```bash
# 예: 다음 버전이 version: 0.0.2+10인 경우
git tag -a v0.0.2 -m "Noteez 0.0.2"
git push origin v0.0.2
```

태그가 올라오면 GitHub Actions가 다시 분석·테스트하고 universal macOS 앱을
ad-hoc 서명한 뒤, 버전에 맞는 DMG와 SHA-256 체크섬을 GitHub Release에
...
전체 보기의 추천 계산은 별도 isolate에서 수행합니다.
...
- 현재 배포 앱 UI는 한국어이며, 영어 스크린샷은 문서용 번역 미리보기입니다.
- Intel Mac·최소 지원 OS와 일부 네이티브 입력·붙여넣기·재시작·절전 후 알림의 상세 실기 검증은 미완료입니다. [릴리스 안내](https://github.com/hofe7/noteez/releases/tag/v0.0.1)를 확인해 주세요.
